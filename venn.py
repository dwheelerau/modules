...
#                                                   #
# https://github.com/icetime/pyinfor/commits/master   #
# Purpose:                                          #
#                                                   #
# Usage: venn([[1,2,3],[2,3,5,6]],names=['list1','list2']) #
#will create a venndia (1)(2)(2) ie overlaps in lists#
#####################################################
import pylab
...
def venn(data, names=None, fill="number", show_names=True, show_plot=True, **kwds):
    """
    kwds: can be title="test",figsize = (5,10) #(x,y), color
    data: a list [[1,2,3,4],[3,4,5,6]],names=['list1','list2']
    names: names of groups in data
    fill = ["number"|"logic"|"both"], fill with number, logic label, or both
    show_names = [True|False]
    show_plot = [True|False] if False will save at 300dpi as venn_figure.png
    """
...
def venn2(data=None, names=None, fill="number", show_names=True, show_plot=True, **kwds):
    ...
    ## draw text
    #1 #add fontsize
    pylab.text(x1-r/2, y1, labels['10'], fontsize=18, **alignment)
    pylab.text(x2+r/2, y2, labels['01'], fontsize=18, **alignment)
    # 2
    pylab.text((x1+x2)/2, y1, labels['11'], fontsize=18,**alignment)
    ...
    if show_plot:
        ...
        pylab.savefig("venn.png",dpi=dpi)
...
def venn3(data=None, names=None, fill="number", show_names=True, show_plot=True, **kwds):
    ...
    ## draw text
    # 1
    pylab.text(x1-r/2, y1-r/2, labels['100'], fontsize=18,**alignment)
    pylab.text(x2+r/2, y2-r/2, labels['010'], fontsize=18,**alignment)
    pylab.text((x1+x2)/2, y3+r/2, labels['001'], fontsize=18,**alignment)
    # 2
    pylab.text((x1+x2)/2, y1-r/2, labels['110'], fontsize=18,**alignment)
    pylab.text(x1, y1+2*r/3, labels['101'], fontsize=18,**alignment)
    pylab.text(x2, y2+2*r/3, labels['011'], fontsize=18,**alignment)
    # 3
    pylab.text((x1+x2)/2, y1+r/3, labels['111'], fontsize=18,**alignment)
    # names of different groups
    if show_names:
    ...
    if show_plot:
        ...
        pylab.savefig("venn_figure.png",dpi=dpi)
